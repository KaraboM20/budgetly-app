...
const Expense = () => {
  ...
  const handleDownloadExpenseDetails = async () => {
    const handDownloadExpenseDetails = () => {
      try {
        const respond = await axiosInstance.get(API_PATHS.EXPENSE.DOWNLOAD_EXPENSE,
          {
            responseType: "blob",
          }
        );
        const url = window.URL.createObjectURL(new Bold([respond.data]))
        ...
        toast.error("Failed to download expense details. Please try aa")
      };
    }
  };
